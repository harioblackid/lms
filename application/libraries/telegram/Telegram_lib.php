...
class Telegram_lib
{
    ...
    {
        $setting = $this->ci->db->get_where('setting', ['id_setting' => 1])->row_array();
        $telegram = new Telegram($setting['bot_token']);
        //$chat_id = $this->ci->config->item('chat_id', 'telegram');
        $content = array('chat_id' => $chatid, 'text' => $msg);
    ...
    public function sendimg($chatid, $img_path, $caption = false)
    {
        $setting = $this->ci->db->get_where('setting', ['id_setting' => 1])->row_array();
        $telegram = new Telegram($setting['bot_token']);
        //$chat_id = $this->ci->config->item('chat_id', 'telegram');
        $img = curl_file_create('test.png', 'image/png');
        $content = ['chat_id' => $chatid, 'photo' => new CURLFile(realpath($img_path)), 'caption' => $caption];
    ...
    public function sendvenue($chat_id, $lat, $long, $title, $address)
    {
        $setting = $this->ci->db->get_where('setting', ['id_setting' => 1])->row_array();
        $telegram = new Telegram($setting['bot_token']);
        //$telegram = new Telegram($this->ci->config->item('bot_token', 'telegram'));
