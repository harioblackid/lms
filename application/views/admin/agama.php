    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header">
                <button class="btn btn-primary" onclick="pilihform('tambah')" data-toggle="modal" data-target="#tambahagama"><i class="fas fa-fw fa-plus"></i> Tambah agama</button>
            </div>
            <!-- /.card-header -->
            <div class="card-body">
...
<div class="modal fade" id="tambahagama" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
...
<script type="text/javascript">
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (data == 'sukses') {
                    ...
                    $('#tableagama').DataTable().ajax.reload();
                    $("[name='nama']").val('');
    ...
    // Delete Records
    $('#tableagama').on('click', '.hapus', function() {
        var id = $(this).data('id');
        console.log(id);
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    url: "<?php echo base_url(); ?>agama/hapusdata",
                    method: "POST",
                    dataType: 'json',
                    data: {
                        id: id
    ...
    //**end**
    // AMBIL ID data table
    $('#tableagama').on('click', '.edit', function() {
        var id = $(this).data('id');
        pilihform();
        $('#tambahagama').modal('show');
        $.ajax({
            url: "<?php echo base_url(); ?>agama/ambilid",
            method: "POST",
            dataType: 'json',
            data: {
                id: id
...
<script>
    ...
    $(document).ready(function() {
        ...
        var table = $("#tableagama").dataTable({
            ...
            bserverSide: true,
            ajax: {
                "url": "<?php echo base_url('agama/get_idt_agama') ?>",
                "type": "POST"
            },
            columns: [{
                    "data": null,
